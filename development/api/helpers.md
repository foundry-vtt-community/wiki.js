...
- The main argument, `fields`, takes a pointer to the actual DataField instance it's rendering
  ...
  - For `formGroup`, the field will add a `label` and/or a `hint` if the corresponding property is present. You can automatically assign these with the [LOCALIZATION_PREFIXES](/en/development/api/localization#localization_prefixes) static property.
